...
When configured, `tina4 serve` mounts `GET /auth/login`, `GET /auth/callback`, and `POST /auth/logout`. Route collisions fail startup. Existing secured routes receive the identity through `request.user`.
...
Provider tokens remain in reserved Session data and never appear in `session.all()`. The runtime does not name providers; Keycloak and Microsoft Entra ID appear only as configuration recipes.
