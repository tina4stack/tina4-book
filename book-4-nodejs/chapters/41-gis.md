...
Declare the model field as `location: { type: "point", srid: 4326, spatialIndex: true }`. Tina4 maps it to `geography(Point,4326)` and creates a GiST index. Unsupported databases fail clearly. `Point.parse()` accepts coordinate pairs, WKT/EWKT, GeoJSON, or WKB/EWKB and never guesses coordinate order.
