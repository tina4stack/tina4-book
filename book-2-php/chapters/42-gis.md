...
Declare the model field with `public ?Point $location = null` and add `'location' => ['srid' => 4326, 'spatialIndex' => true]` to `$pointFields`. Tina4 creates `geography(Point,4326)` and a GiST index. Unsupported databases fail clearly. `Point::parse()` accepts coordinate pairs, WKT/EWKT, GeoJSON, or WKB/EWKB.
