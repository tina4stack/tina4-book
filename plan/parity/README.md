...
- PHP WebSocket was flagged as missing, but it exists (full RFC 6455)
- Ruby/Node Api Client flagged as missing, but both exist (full HTTP client)
- Node Queue backends flagged as missing, but all 4 backends exist
- Template methods flagged as missing in Ruby/Node, but the Frond class has them all
- Queue method naming flagged, but all use push/pop/consume
