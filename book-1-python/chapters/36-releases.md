...
- The dev dashboard queue panel lists the store it counts (`f539be2`)
- The AMQP vhost is read as the path segment (`6f4ab2a`)

### Logging

- An explicit argument beats the environment, which beats the default (`c56d3a1`)
...
- Firebird: `commit`/`rollback` with no open transaction are no-ops, not crashes (`4b2e38d`)

### Sessions

- A backend failure is loud, then degrades, on the request path too (`59bee59`)
- No network I/O in a handler constructor (`f423ec5`)
- Every backend has a zero-dependency transport (`7e8416f`)
- `TINA4_SESSION_REDIS_PREFIX` and `_VALKEY_PREFIX` are honoured (`f7a5ba2`)
- The database backend's DDL is portable to MySQL (`c0886f5`)

### Document store and cache

- `find()` is deferred, and there is one sort contract (`cae5484`, `0b2dfc1`)
- A missing MongoDB driver raises instead of falling through (`219fa36`)
- memcached TTL beyond 30 days no longer vanishes instantly (`f1abf81`)
- redis `stats()` size was a constant on the raw transport (`8c0cd5b`)
- `boto3` is declared, so a sync cannot silently delete live-S3 coverage (`fbd7dd2`)
...
`primary_key()` took `next(c for c in columns if c.primary_key)`, the FIRST key
column. On a composite key that reintroduced the exact bug the change existed to
prevent. Verified against real SQLite on `order_items (order_id, product_id)`: the
update popped `order_id`, built `WHERE order_id = 1`, and tried to set
`product_id = 5` on both rows in that group. A UNIQUE constraint caught it. Without
one it would have corrupted both rows in silence.

The Kafka raw path only runs when `confluent-kafka` is absent, and CI installs it,
so that code had never executed under test. It does now: a test forces the raw path
against a real broker. CRC32C is built in-process, because the standard library has
none and the framework carries no third-party runtime dependency.

The published 3.13.92 image served `"version": "3.13.56"` on `/health`, 36 releases
stale. `_resolve_version()` falls back from `pyproject.toml` to
`importlib.metadata`, then to a floor literal. The Dockerfile pruned `dist-info`,
which removed the only metadata path, so the literal won. The literal had never
been bumped, because in a source checkout the first path always wins and no test
ever reached it. The prune bought 20 KB of a 41 MB image. It is gone, and a test
now reads the literal out of the source and checks it against `pyproject.toml`.

**Frond:** 14.5x slower than Jinja2 and 37x slower than Mako on identical output.
The harness confirms the AOT compiler engaged, so this is Frond's fastest path.
...
note records that the other Tina4 frameworks were brought in line: PHP (which used to
return a bool) and Ruby (which used to return a Hash) now return a result object from
these writes too, so a write returns a result in every language.
...
## v3.13.30 (2026-06-16) - Typed route params now arrive coerced (Warning: behavioural change)
...
### Warning: Breaking - one database binder: `bind_database`
...
|---|---|---|---|---|
| `fetch_all` returns ALL rows by default | [x] `limit=0` default | [x] `$limit = 0` default | [x] `limit: nil` default | [x] already correct (`limit?` undefined) |
...
| Fix | Python | PHP | Ruby | Node |
|---|---|---|---|---|
| #50.1 callable defaults | [x] fixed | N/A (PHP property defaults are constants) | [x] fixed (Procs) | N/A (no auto-default at construction) |
| #50.2 natural-key INSERT | [x] fixed | [x] already correct (`recordExists`) | [x] already correct (`@persisted` flag) | [x] fixed |
| #49.1 + #49.2 PG visibility | [x] fixed (Python-only - libpq autocommit means cascade never happens on PHP/Ruby/Node) |  |  |  |
| BooleanField DDL | [x] fixed | N/A (PHP createTable is migration-driven) | [x] fixed | [x] already engine-aware |
...
- **feat:** Language toggle - flag button (FR/GB) in navbar to switch locale.
